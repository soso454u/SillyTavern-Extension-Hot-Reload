...
从 `1.3.1` 起，更新器检测到入口引用扩展目录内的本地子模块时，会自动改走无感重启，不再报告为直接热载入成功。`1.4.0` 起同样会检测 `new URL(..., import.meta.url)`、`Worker` 和 `SharedWorker` 声明的本地运行资源。扩展作者若希望保留真正的直接热更新，可靠的发布方式有两个：
...
- Git pull 已经成功时，任何后续生命周期失败都会降级到无感重启，确保最终加载磁盘上的完整新版本；
