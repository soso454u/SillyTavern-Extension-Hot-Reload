...
const RESTART_QUERY_KEY = 'st_hot_restart';
...
/**
 * Compare restart locations without the one-shot navigation token.
 * @param {string|URL} value
 * @returns {string}
 */
export function normalizeRestartPath(value) {
    const url = new URL(value, 'http://sillytavern.local');
    url.searchParams.delete(RESTART_QUERY_KEY);
    return `${url.pathname}${url.search}${url.hash}`;
}

/**
 * Force a fresh same-origin document navigation without adding a history item.
 * @param {string|URL} value
 * @param {string} token
 * @returns {string}
 */
export function buildRestartNavigationUrl(value, token) {
    const url = new URL(value, 'http://sillytavern.local');
    url.searchParams.set(RESTART_QUERY_KEY, String(token));
    return url.href;
}

/**
 * Remove the one-shot navigation token after the new document starts.
 * @param {string|URL} value
 * @returns {string}
 */
export function stripRestartNavigationToken(value) {
    const url = new URL(value, 'http://sillytavern.local');
    url.searchParams.delete(RESTART_QUERY_KEY);
    return url.href;
}
